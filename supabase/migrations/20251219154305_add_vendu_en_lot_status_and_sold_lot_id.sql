/*
  # Correctif anti double-comptage : Lots vs Articles

  1. Modifications
    - Ajoute le statut 'vendu_en_lot' (stocké en text dans la colonne status)
    - Ajoute la colonne `sold_lot_id` aux articles pour tracer le lot de vente
    - Empêche le double comptage en séparant les ventes de lots et ventes individuelles

  2. Nouveaux champs
    - `sold_lot_id` (uuid nullable) : ID du lot qui a vendu l'article

  3. Règles métier
    - Un article vendu_en_lot ne doit plus être vendable individuellement
    - Le CA d'un lot vendu est porté par le lot, pas par ses articles
    - Les stats doivent distinguer ventes de lots et ventes individuelles

  4. Sécurité
    - Les politiques RLS existantes s'appliquent automatiquement

  Note: Cette migration est idempotente et ne s'exécutera que si les tables existent
*/

-- Ajouter la colonne sold_lot_id si la table articles existe
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'articles') THEN
    IF NOT EXISTS (
      ...
      -- Ajouter la contrainte de clé étrangère si la table lots existe
      ...
      CREATE INDEX idx_articles_sold_lot_id ON articles(sold_lot_id);
