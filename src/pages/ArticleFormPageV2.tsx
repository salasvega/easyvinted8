...
import { getDefaultSellerAvatarAndLocation } from '../services/settings';
...
export function ArticleFormPageV2() {
  ...
  const loadUserProfile = async () => {
    if (!user) return;
    try {
      const { writingStyle } = await getDefaultSellerAvatarAndLocation(user.id);
      setUserProfile({ writing_style: writingStyle });
    } catch (error) {
