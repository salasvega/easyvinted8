...
import { getDefaultSellerAvatarAndLocation, getUserLocations, getUserAvatars, LocationData, AvatarData } from '../services/settings';
...
export function PhotoStudioPage() {
  ...
  const [userAvatars, setUserAvatars] = useState<AvatarData[]>([]);
  ...
  useEffect(() => {
    const loadDefaultSellerData = async () => {
      if (!user?.id) return;
      try {
        const { writingStyle: style } = await getDefaultSellerAvatarAndLocation(user.id);
  ...
  const handleImageSelected = useCallback(async (base64: string, type: string) => {
    ...
    try {
      const detectedProducts = await analyzeProductImage(base64, type, writingStyle || undefined);
